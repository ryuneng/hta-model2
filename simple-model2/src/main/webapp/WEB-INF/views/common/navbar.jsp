...
<nav class="navbar navbar-expand-sm bg-light">
	<div class="container">
		...
		<c:if test="${not empty LOGIN_USER }">
			<span class="navbar-text"><strong>${LOGIN_USER.name }</strong>님 환영합니다.</span>
		</c:if>
		
		<ul class="navbar-nav">
		<c:if test="${empty LOGIN_USER }"> <!-- LOGIN_USER가 비어있을 때 -->
			<li class="nav-item">
				<a class="nav-link ${menu eq '로그인' ? 'active' : '' }" href="/login.do">로그인</a>
			</li>
		</c:if>
		<c:if test="${not empty LOGIN_USER }"> <!-- LOGIN_USER가 비어있지 않을 때 -->
			<li class="nav-item">
